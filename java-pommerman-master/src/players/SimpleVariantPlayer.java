...
public class SimpleVariantPlayer extends Player {
     ...
     */
    public SimpleVariantPlayer(long seed, int id) {
        super(seed, id);
        reset(seed, id);
    }
    ...
    @Override
    public Player copy() {
        SimpleVariantPlayer player = new SimpleVariantPlayer(seed, playerID);
    ...
    public Types.ACTIONS act(GameState gs) {
        ...
        if (validDirections.size() > 0) {
            ...
            for (Types.DIRECTIONS direction : validDirections) {
                GameState gsCopy = gs.copy();
                Types.ACTIONS action = directionToAction(direction);
